...
const priorityConfig = {
  low: { label: 'Low', class: 'text-priority-low border-priority-low/30 bg-priority-low/10' },
  medium: { label: 'Medium', class: 'text-priority-medium border-priority-medium/30 bg-priority-medium/10' },
  high: { label: 'High', class: 'text-priority-high border-priority-high/30 bg-priority-high/10' },
};
  ...
  return (
    <div
      className={cn(
        ...
        isCompleted ? 'border-border/40 opacity-60' : 'border-border hover:border-border-subtle',
        isOverdue && !isCompleted && 'border-status-overdue/20 bg-status-overdue/5'
      )}
    >
      {/* Checkbox */}
      <button
